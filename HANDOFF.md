...
- 功能 commit `1b30864` 已推送至既有功能分支並部署；Worker version
  `e13e4369-8380-4d8f-ba29-c0339e87e0a6`。本次不改 schema／正式清單／料檔，前台 shell 仍 v55。
  發布後唯讀驗證首頁 200、管理頁／JS／API 匿名 302 至 Access、匿名料檔 API 401。
  正式 D1 額度已耗盡，因此未重跑讀庫驗收，不得把程式發布成功誤報成資料庫已恢復；
  如方案未調整，下次重置為台灣時間 2026-09-18 08:00，待之後驗證。

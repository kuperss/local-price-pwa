...
const trackedCte=catalogCte('SELECT sku FROM catalog');
const sourceCte=catalogCte("SELECT sku FROM product_metadata WHERE snapshot=(SELECT value FROM settings WHERE key='product_snapshot') UNION SELECT sku FROM catalog");
const lookupCte=catalogCte('SELECT value AS sku FROM json_each(?)');
...
const product=async(db,sku)=>(await productMap(db,[sku])).get(sku);
...
export async function catalogApi(request,db,url,data,actor){
 ...
 if(request.method==='GET'&&!path){
  ...
  for(const row of rows)row.attentionReasons=attentionReasons(row);

...
- A2 外倉為 0、在途為 0／未列在途都正常，不單獨列入「需要檢查」，亦不標示警告色。
  在途缺值顯示「未列在途」，不是已確認 0；A2 缺值仍是未知，不能影響可用量計算規則。
  「需要檢查」是清單維護提醒，不等於庫存異常；選取時必須逐筆顯示檢查原因。
  有庫存的舊品也可能因替代型號未納入啟用清單而列入，不能僅看庫存判斷誤報。
- 2026-09-17 發現 D1 帳號免費每日讀取額度耗盡（code 7500、daily row read limit）。
  單型號／替代查詢以 JSON 型號清單做主鍵 join；清單統計只掃 catalog，不可恢復完整來源 UNION 的查詢方式。
  完整來源檢索才使用全來源；額度錯誤回 503 與明確提示，不能繞過授權或當作空清單／停權。
  免費額度台灣每日 08:00 重置；部署不會恢復已用完額度，不擅自升級付費或反覆查正式 D1。
  唯讀診斷也消耗額度；check_catalog 預設僅查啟用清單，指定 SKU 查明細，只有 --all 才全面檢查。
...
- 售轉串接僅依 ERP 與管理員手動關係，不猜測料號版次；允許一對多，阻擋新手動循環。
  清單售轉欄直接顯示替代品實際可用量（有貨／無貨／未知），每列「取代」開獨立確認視窗。
  確認取代的 targets 是明確選定的目的型號；publisher 對 replace_old 且 targets 非空時以此為準，
  不強制把未選的 ERP 售轉型號加回，但 ERP 原始關係仍保留供查閱。

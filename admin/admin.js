...
async function load(){
 ...
 try{
  const [overview,rows]=await Promise.all([api('overview'),api(page+'?'+new URLSearchParams({offset,device,query}))]);if(token!==serial)return;
...
function show(next){page=next;offset=0;document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===page));$('#title').textContent={devices:'裝置與使用者',events:'查詢紀錄',requests:'型號審核',catalog:'更新清單'}[page];
